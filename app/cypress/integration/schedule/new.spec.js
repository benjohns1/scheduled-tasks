		...
		it('creates an editable schedule form at the top', () => {
			cy.get('[data-test=schedules]').then($t => $t.find('[data-test=schedule-item]').length).then(startingCount => {
				...
				cy.get('[data-test=schedule-item]').first().then($s => {
					...
					cy.wrap($s).find('[data-test=schedule-name]').should('have.text', '[new unsaved schedule]');
					cy.wrap($s).find('[data-test=schedule-frequency-input]').should('have.value', 'hourly');
					...
					cy.get('[data-test=schedule-item]').should('have.length', expectedCount);
					cy.wrap($s).find('[data-test=schedule-name]').should('have.text', 'every hour at :00');
					cy.wrap($s).find('[data-test=schedule-frequency]').should('have.text', 'hourly');
