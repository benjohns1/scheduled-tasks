...
func Test_addListGetScheduleAndAddRemoveRecurringTasks(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			asserts: asserts{statusEquals: http.StatusOK, bodyEquals: strp(`{"id":1,"frequency":"hourly","atMinutes":[],"paused":false,"tasks":[]}`)},
		},
		{
			name:    "get schedule ID 2 should return hourly schedule with no recurring tasks",
			h:       api,
			args:    args{method: "GET", url: "/api/v1/schedule/2"},
			asserts: asserts{statusEquals: http.StatusOK, bodyEquals: strp(`{"id":2,"frequency":"hourly","atMinutes":[0,30],"paused":true,"tasks":[]}`)},
		},
		{
			name:    "get schedule ID 3 should return hourly schedule with 1 recurring tasks",
			h:       api,
			args:    args{method: "GET", url: "/api/v1/schedule/3"},
			asserts: asserts{statusEquals: http.StatusOK, bodyEquals: strp(`{"id":3,"frequency":"hourly","atMinutes":[0,30,59],"paused":false,"tasks":[{"name":"rtask1","description":"rtask1 desc"}]}`)},
		},
		{
			name:    "should return 200 list with 3 schedules",
			h:       api,
			args:    args{method: "GET", url: "/api/v1/schedule/"},
			asserts: asserts{statusEquals: http.StatusOK, bodyEquals: strp(`{"1":{"id":1,"frequency":"hourly","atMinutes":[],"paused":false,"tasks":[]},"2":{"id":2,"frequency":"hourly","atMinutes":[0,30],"paused":true,"tasks":[]},"3":{"id":3,"frequency":"hourly","atMinutes":[0,30,59],"paused":false,"tasks":[{"name":"rtask1","description":"rtask1 desc"}]}}`)},
		},
		{
			...
			asserts: asserts{statusEquals: http.StatusCreated, bodyEquals: strp(``)},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
